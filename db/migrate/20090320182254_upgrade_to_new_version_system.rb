class UpgradeToNewVersionSystem < ActiveRecord::Migration
  ...
  def self.up
    remove_column(:parselets, :cached_changes) if Parselet.columns.include?('cached_changes')
    ...
    Parselet.transaction do
      ...
      versions.each do |version|
        ...
        (attributes.keys - ["parselet_id", "pattern_regex", "id", "cached_rating", "comments_count", "ratings_count"]).each do |key|
