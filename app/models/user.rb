...
class User < ActiveRecord::Base
  module ClassMethods
    def top(n = 5)
      find_by_sql "SELECT users.*, COUNT(parselet_versions.id) as p FROM users, parselet_versions WHERE parselet_versions.user_id = users.id AND parselet_versions.works=1 GROUP BY parselet_versions.parselet_id ORDER BY p DESC LIMIT #{n.to_i}"
    end
