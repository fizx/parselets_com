...
class User < ActiveRecord::Base
  has_many :sprigs
  has_many :parselets
  has_many :revised_parselets, :class_name => 'Parselet', :foreign_key => 'revision_user_id'
  ...
  def self.top(n = 5)
    find_by_sql "SELECT users.*, COUNT(parselets.id) as p FROM users 
      LEFT JOIN parselets ON parselets.revision_user_id = users.id WHERE parselets.works=1 GROUP BY parselets.revision_user_id ORDER BY p DESC LIMIT #{n.to_i}"
  end
